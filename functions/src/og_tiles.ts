...
export type SizePad = { w: number; h: number; pad: number };
...
export function fitBoundsToCenterZoom(bbox: BBox, size: SizePad): { center: { lat: number; lng: number }; zoom: number } {
  ...
  if (!Number.isFinite(z)) z = 14; // tek nokta vs.
...
export function buildMapTilerUrl(p: {
  ...
  return `https://api.maptiler.com/maps/${style}/static/${p.center.lng},${p.center.lat},${z}/${w}x${h}.png?key=${encodeURIComponent(
    p.key
  )}&attribution=0&logo=0`;
}

export async function fetchTilePng(url: string): Promise<Buffer> {
...
export function lighten(buf: Buffer, _amount = 0.08): Buffer {
  return buf;
}
