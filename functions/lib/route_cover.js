...
exports.backfillRouteCoversCallable = exports.onRouteWriteEnsureCoverDefault = exports.onRouteStopMediaCreate = exports.onRouteStopCreate = void 0;
...
function pickStopTitle(stop) {
    const t = (typeof stop?.title === "string" && stop.title) ||
        (typeof stop?.name === "string" && stop.name) ||
        (typeof stop?.label === "string" && stop.label) ||
        "";
    return String(t).trim();
}
function buildStopPreview(stopId, stopData) {
...
function pickMediaUrls(media) {
    const coverUrl = (isNonEmptyString(media?.url) && media.url) ||
        ...
        "";
    const thumbnailUrl = (isNonEmptyString(media?.thumbnailUrl) && media.thumbnailUrl) ||
...
function deriveTotalDistanceM(route) {
...
function shouldAutoReplaceCover(route) {
    const cs = (route?.coverSource || "").toString();
    ...
    if (cs === "default")
        return true;
    if (isNonEmptyString(coverUrl) && coverUrl.startsWith("data:image/svg+xml"))
        return true; // bizim default
    return false;
}
function ensureDefaultCoverPatch(route) {
    const coverUrl = route?.coverUrl;
    ...
    if (cs === "manual" && isNonEmptyString(coverUrl)) {
        const patch = {};
        if (!isNonEmptyString(thumbUrl))
            patch.thumbnailUrl = coverUrl;
        if (!isNonEmptyString(route?.coverUpdatedAt))
    ...
    if (isNonEmptyString(coverUrl) && !isNonEmptyString(thumbUrl)) {
        return {
            ...
            coverUpdatedAt: admin.firestore.FieldValue.serverTimestamp(),
            coverSource: (cs === "manual" ? "manual" : cs || "auto"),
        };
    }
...
/* =========================
   ...
   ========================= */
exports.onRouteStopCreate = functions
    .region("us-central1")
    .firestore.document("routes/{routeId}/stops/{stopId}")
    .onCreate(async (snap, ctx) => {
    const { routeId, stopId } = ctx.params;
    ...
    await db.runTransaction(async (t) => {
        ...
        const existingPreview = Array.isArray(r.stopsPreview) ? r.stopsPreview : [];
        ...
        if (existingPreview.length === 0)
            nextPreview = [stopPreview];
        else if (existingPreview.length === 1)
            nextPreview = [existingPreview[0], stopPreview];
        else
            nextPreview = [existingPreview[0], stopPreview];
        ...
        if (!isNonEmptyString(r.startName) && isNonEmptyString(startName))
            patch.startName = startName;
        if (!isNonEmptyString(r.endName) && isNonEmptyString(endName))
            patch.endName = endName;
        ...
        if (coverPatch)
            Object.assign(patch, coverPatch);
...
/* =========================
   ...
   ========================= */
exports.onRouteStopMediaCreate = functions
    .region("us-central1")
...
/* =========================
   ...
   ========================= */
exports.onRouteWriteEnsureCoverDefault = functions
    .region("us-central1")
    ...
    const afterCover = after?.coverUrl;
    const patchCover = patch?.coverUrl;
...
/* =========================
   ...
   ========================= */
exports.backfillRouteCoversCallable = functions
    .region("us-central1")
    ...
    for (let page = 0; page < 8; page++) {
        ...
        for (const doc of snap.docs) {
            ...
            try {
                ...
                let firstStopSnap = null;
                let lastStopSnap = null;
                try {
                ...
                const preview = [];
                if (firstStopDoc)
                    preview.push(buildStopPreview(firstStopDoc.id, firstStopDoc.data() || {}));
                ...
                if (cs === "manual" && isNonEmptyString(r.coverUrl)) {
                    ...
                    if (!isNonEmptyString(r.thumbnailUrl))
                        patch.thumbnailUrl = r.coverUrl;
                    if (!isNonEmptyString(r.coverUpdatedAt))
                        patch.coverUpdatedAt = admin.firestore.FieldValue.serverTimestamp();
                }
                else {
