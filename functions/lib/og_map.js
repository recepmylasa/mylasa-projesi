...
function approxBBoxMeters(latlngs) {
    if (!latlngs.length)
        return 0;
    ...
    const latMeters = (maxLat - minLat) * 111320; // ~m/deg
    const lonMeters = ((maxLng - minLng) * 111320 * Math.cos((midLat * Math.PI) / 180)) || 0;
