...
async function reverseGeocode(lat, lng) {
    ...
    try {
        if (provider === "opencage") {
            if (!key)
                throw new Error("OpenCage API key missing");
            const url = `https://api.opencagedata.com/geocode/v1/json?q=${lat}+${lng}&key=${encodeURIComponent(key)}&no_annotations=1&language=tr`;
        ...
        // default: google
        if (!key)
            throw new Error("Google Geocoding API key missing");
        const url = `https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key=${encodeURIComponent(key)}&language=tr&result_type=locality|administrative_area_level_1|country`;
        const r = await fetch(url);
        const j = await r.json();
