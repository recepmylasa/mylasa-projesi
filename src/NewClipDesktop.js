...
// İkonlar
const VideoIcon = () => (
  <svg aria-label="Video ikonu" fill="currentColor" height="77" role="img" viewBox="0 0 96 77" width="96">
    <path d="M72.2 24.2H65.5l-5.8-8.2H36.3l-5.8 8.2H23.8C15.1 24.2 8 31.3 8 40v19c0 8.7 7.1 15.8 15.8 15.8h48.5c8.7 0 15.8-7.1 15.8-15.8V40c0-8.7-7.1-15.8-15.8-15.8Zm-2.2 28.5L55 59.8V42.2l15 7.1a1 1 0 0 1 0 1.8Z" />
  </svg>
);
const BackArrowIcon = () => (
  <svg aria-label="Geri" height="24" role="img" viewBox="0 0 24 24" width="24">
    <path
      d="M21 11.3H5.7l6.2-6.2c.5-.5.5-1.3 0-1.8s-1.3-.5-1.8 0l-8.1 8.1c.5.5-.5 1.3 0 1.8l8.1 8.1c.5.5 1.3 .5 1.8 0s.5-1.3 0-1.8l-6.2-6.2H21c.7 0 1.3-.6 1.3-1.3s-.6-1.3-1.3-1.3z"
      fill="currentColor"
    />
  </svg>
);
...
function NewClipDesktop({ onClose }) {
  ...
  const handlePaylas = async () => {
    if (!kullanici || !videoFile) {
      setHataMesaji("Kullanıcı veya video dosyası bulunamadı.");
      return;
    }
    ...
    <div className="new-clip-overlay" onClick={onClose}>
      <div className={`new-clip-modal-content step-${step}`} onClick={(e) => e.stopPropagation()}>
        <header className="new-clip-header">
          {step === 2 && (
            <button onClick={handleGeri} className="header-btn back" disabled={isUploading}>
              <BackArrowIcon />
            </button>
          )}
          <h3>{step === 1 ? "Yeni Clip Oluştur" : "Yeni Reels videosu"}</h3>
          {step === 2 && (
            <button onClick={handlePaylas} className="header-btn share" disabled={isUploading}>
        ...
        <div className="new-clip-body">
          {step === 1 && (
            <div className="file-select-container">
              ...
              />
              <VideoIcon />
              <p>Clips videolarını buraya sürükle</p>
              <label onClick={() => fileInputRef.current.click()} className="file-select-btn">
                Bilgisayardan seç
              </label>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="clip-editor-container">
              <div className="clip-video-preview-wrapper">
                {videoPreviewUrl && (
                  <video ref={videoRef} className="clip-video-preview" autoPlay muted loop playsInline>
