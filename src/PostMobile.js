...
function PostMobile({ post, aktifKullaniciId, onUserClick, onCommentClick }) {
  ...
  const visibleScore = typeof rep?.visible === 'number'
    ? rep.visible : (rep?.visible ? Number(rep.visible) : 0);
  const sample = typeof rep?.sample === 'number'
    ...
    <article className="m-post-article">
      ...
      <div className="m-post-content">
        ...
        {post?.yorumlar?.length > 0 && (
          <button
            className="m-post-comments-link"
            onClick={() => onCommentClick?.(post)}
          >
