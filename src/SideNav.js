...
function SideNav({ activeTab, onTabChange, profilePic }) {
    ...
    return (
        <nav className="sidenav-container">
            {/* --- LOGO GÜNCELLENDİ (Stiller CSS'e taşındı) --- */}
            <div className="sidenav-logo-container" onClick={() => window.location.reload()}>
                <img src="/mylasa-logo.png" alt="Mylasa İkon" className="sidenav-logo-icon" />
                    ...
                    <li key={link.id}>
                        <a onClick={() => onTabChange(link.id)} className={`sidenav-link ${activeTab === link.id ? 'active' : ''}`}>
                            <span className="sidenav-icon">
                                {link.profile ? (
                                    profilePic ? <img src={profilePic} alt="Profil" className="bottom-nav-profile-pic" /> : <div className="bottom-nav-profile-pic" style={{background:'#e0e0e0'}}></div>
                                ) : (
