// FILE: src/pages/RouteDetailMobile/components/RouteDetailCoverPickerOverlayMobile.js
          ...
          <>
            {state?.error && (
              <div className="route-detail-cover-picker-error">
                Kapak medyaları yüklenemedi. ({state.error})
              </div>
            )}
                ...
                {(state?.items || []).map((it) => (
                  ...
                  >
                    <img
                      ...
                      decoding="async"
                      onLoad={(e) =>
                        onImgLoad?.(e, { scope: "cover_picker", stopId: it.stopId, mediaId: it.mediaId || it.id || null })
                      }
                      onError={(e) =>
                        onImgError?.(e, { scope: "cover_picker", stopId: it.stopId, mediaId: it.mediaId || it.id || null })
                      }
                    />
                  </button>
                ))}
