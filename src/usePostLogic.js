...
export function usePostLogic(post, aktifKullaniciId, onCommentClick) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(userRef, (s) =>
      setAuthorProfile(s.exists() ? s.data() : null)
    );
    return () => unsub();
  }, [post?.authorId]);
  ...
  // Menü dışına tık/escape/scroll kapat
  useEffect(() => {
    if (!optionsOpen) return;
    const onDown = (e) => e.key === "Escape" && setOptionsOpen(false);
    const onClickAway = (e) =>
      menuRef.current && !menuRef.current.contains(e.target) && setOptionsOpen(false);
  ...
  const { visibleScore, showGold } = useMemo(() => {
    ...
    const visible =
      typeof rep?.visible === "number"
        ? rep.visible
        : rep?.visible
        ? Number(rep.visible)
        : 0;
    const sample =
      typeof rep?.sample === "number"
        ? rep.sample
        : rep?.sample
        ? Number(rep.sample)
        : 0;
  ...
  const handleDelete = async () => {
    ...
    try {
      if (post.mediaStoragePath) {
        await deleteObject(ref(storage, post.mediaStoragePath));
      }
  ...
  const handleToggleComments = async () => {
    ...
    try {
      await updateDoc(doc(db, "posts", post.id), {
        yorumlarKapali: !post?.yorumlarKapali,
      });
