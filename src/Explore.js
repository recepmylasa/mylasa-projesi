...
const isClipItem = (it) => {
  const t = (it?.type || it?.format || it?.kind || "")
    .toString()
    .toLowerCase();
  const mt = (it?.mediaType || it?.mime || it?.mimeType || "")
    .toString()
    .toLowerCase();
...
function Explore({ aktifKullaniciId, onUserClick }) {
  ...
  /* ——— Kullanıcı araması (debounce + Firestore) ——— */
  useEffect(() => {
    let alive = true;
  ...
  return (
    <>
      <div className="explore-container">
        ...
        {/* Normal Explore: kullanıcı araması veya gönderi grid’i */}
        {searchTerm.trim() ? (
          ...
          <div className="explore-grid" role="list">
            ...
            {/* Sonsuz kaydırma sentinel + ek skeletonlar */}
            {!isEnd && (
              <>
...
/* ——— yardımcı: post listelerini birleştir ——— */
